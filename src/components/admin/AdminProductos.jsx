...
export default function AdminProductos({ comercio }) {
  ...
  const generateDescriptions = useMutation({
    mutationFn: async (style = 'creativo') => {
      const response = await commerceClient.post('generarDescripcionesProducto', {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingProduct) {
        await commerceClient.post('actualizarProducto', {
          productoId: editingProduct.id,
          productoData: data
        });
      } else {
        await commerceClient.post('crearProducto', {
          productoData: data
        });
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await commerceClient.post('eliminarProducto', { productoId: id });
      queryClient.invalidateQueries(['productos-admin']);
    } catch (err) {
  ...
  const handleToggleActivo = async (producto) => {
    try {
      await commerceClient.post('toggleActivoProducto', {
  ...
  const handleDeleteCategory = async (categoria) => {
    ...
    try {
      const response = await commerceClient.post('eliminarCategoria', {
        commerce_code: comercio.commerce_code,
      ...
      if (response?.success) {
        queryClient.invalidateQueries(['productos-admin']);
        // alert(`Categoría eliminada. ${response.updated} productos actualizados`);
      } else {
  ...
  const processCsvImport = async () => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        try {
          const response = await commerceClient.post('importarProductosMasivo', {
            commerce_code: comercio.commerce_code,
            productos: productosParaImportar
          });

          if (response.success) {
            setImportResult({ success: true, message: response.message, detalles: response.detalles });
            queryClient.invalidateQueries(['productos-admin']);
          } else {
            throw new Error(response.error || 'Error desconocido');
          }
    ...
    const handleBulkPriceChange = async () => {
      try {
        const response = await commerceClient.post('cambioMasivoPrecio', {
        ...
        setSelectedProducts([]);
        alert(`${response.actualizados} productos actualizados`);
      } catch (err) {
