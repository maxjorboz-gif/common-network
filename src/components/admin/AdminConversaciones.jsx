  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      <div className={`max-w-[75%] ${isUser && 'flex flex-col items-end'}`}>
        <div className={`rounded-2xl px-4 py-2.5 ${isUser ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-900'
          }`}>
          {isUser ? (
...
export default function AdminConversaciones({ comercio }) {
  ...
  const { data: conversaciones = [], isLoading } = useQuery({
    queryKey: ['conversaciones-agente', comercio.commerce_code || comercio.id_comercio],
    queryFn: async () => {
      try {
        const convs = await base44.agents.listConversations({
          agent_name: 'AgenteVentas'
        });
        // Client-side filtering to ensure multi-tenant isolation
        const currentCode = comercio.commerce_code || comercio.id_comercio;
        return (convs || []).filter(c => c.metadata?.commerce_code === currentCode || c.metadata?.id_comercio === currentCode);
      } catch (err) {
