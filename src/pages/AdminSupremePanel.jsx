...
// import { superAdminClient } from '@/api/superAdminClient'; // DEPRECATED from v1

// Shim for SuperAdminClient to work with v2 clean architecture
const superAdminClient = {
    post: async (functionName, body) => {
        const { data, error } = await base44.functions.invoke(functionName, body);
        if (error) throw new Error(error.message || "Error desconocido en función");
        return data;
    }
};
...
export default function AdminSupremePanel() {
    ...
    const displayEmail = superAdmin?.email || "Super Admin";
